...
C = 1.0     # changed by hu3 from 1 to 1.0 to make it float
...
for C in Cvals:
    for sigma in sigma_vals:
        ...
        K_val = np.array([utils.gaussian_kernel(x1,x2,sigma) for x1 in Xval for x2 in X]).reshape(Xval.shape[0],X.shape[0])
        scaler_val = preprocessing.StandardScaler().fit(K_val)
        scaleK_val = scaler.transform(K_val)
        KK_val = np.vstack([np.ones((scaleK_val.shape[0],)),scaleK_val.T]).T

        acc = metrics.accuracy_score(yyval, svm.predict(KK_val))
        if acc > accuracy:
            best_C, best_sigma, accuracy = C, sigma, acc
        print "C={}    sigma={}    accuracy={}".format(C, sigma, acc)

print "best_C={}    best_sigma={}    best_accuracy={}".format(C, sigma, accuracy)
...
# best_C=30    best_sigma=30    best_accuracy=0.96
svm = LinearSVM_twoclass()
