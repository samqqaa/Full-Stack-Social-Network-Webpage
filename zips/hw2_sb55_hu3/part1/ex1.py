...
pred_prob = utils.sigmoid(np.dot(theta_opt, np.array([1,45,85])))
...
accuracy = np.mean((predy == y).astype(float))*100
...
print "Accuracy by sklearn = ", np.mean((sk_logreg.predict(XX) == y).astype(float))*100
